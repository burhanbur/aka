...
@section('content')
<form action="{{ route('update.survey', $id) }}" method="POST" enctype="multipart/form-data">
	@csrf
	@method('PUT')

	<div class="card">
		<div class="card-header">
			<h3 class="card-title text-primary">Update Survey</h3>	
		</div>
					...
					<div class="form-group">
						<select name="activity_id" class="form-control" id="activity" required>
							...
							@foreach ($activities as $activity)
								<option @if ($activity->id == $survey->activity_id) selected @endif value="{{ $activity->id }}">{{ $activity->name }}</option>
							@endforeach
						</select>
					</div>
					<div class="form-group">
						<input type="text" class="form-control" name="title" value="{{ $survey->title }}" placeholder="Title survey ..." required>
					</div>
					<div class="form-group">
						<textarea name="description" class="form-control" placeholder="Form description ...">{{ $survey->description }}</textarea>
					</div>
					<div class="row">
						<div class="col-md-6">
						...
						<div class="col-md-6">
							...
							<div class="form-group">
					            <input type="checkbox" name="is_active" @if ($survey->is_active) checked @endif data-bootstrap-switch data-off-color="danger" data-on-color="success">
							</div>
						</div>
					</div>
				</div>
	...
	<div class="card">
		<div class="card-body">
			<!-- list questions -->
			<div id="questions">
				@foreach($survey->question as $key => $value)
				<test>
					<input type="hidden" name="u_question_id[{{ $key }}]" value="{{ $value->id }}">
					<div class="card">
					    <div class="card-header">
					        <div style="float: right;">
					            <a href="" class="btn btn-danger btn-sm delData">
					    ...
					    <div class="card-body">
					        <div class="row">
					            <div class="col-md-8">
					                <div class="form-group">
					                    <textarea name="u_question[{{ $key }}]" class="form-control" placeholder="Question Text ..." required>{{ $value->question }}</textarea>
					                </div>
					                <strong>Answer</strong>
					                <div id="essay{{ $key }}">
				                    	@if ($value->type == 'ESSAY')
					                    	<div class="form-group"> <input type="text" class="form-control" value="Short answer text" readonly> </div>
				                    	@endif
					                </div>
					                <div id="multiple_choice{{ $key }}">
					                    <div id="show_option{{ $key }}">
					                    	@if ($value->type == 'MULTIPLE_CHOICE_CHECK' || $value->type == 'MULTIPLE_CHOICE_RADIO')
					                    		@foreach($value->multipleChoice as $keyOption => $valueOption)
					                    		<test>
					                    			<input type="hidden" name="u_choice_id[{{ $key }}][{{ $keyOption }}]" value="{{ $valueOption->id }}">
					                    			<div class="form-group">
													    <div class="input-group">
													        <input type="text" name="u_answer[{{ $key }}][{{ $keyOption }}]" class="form-control" placeholder="Option" value="{{ $valueOption->answer }}" required>
													        <div class="input-group-prepend">
													            <span class="input-group-text">
													                <a href="javascript:void(0)" class="ibtnDel">
				                    	...
				                    	@if ($value->type == 'MULTIPLE_CHOICE_CHECK' || $value->type == 'MULTIPLE_CHOICE_RADIO')
											<div class="form-group">
											    <a href="javascript:void(0)" class="btn btn-sm btn-warning" onClick="return addUpdateOption({{ $value->id }}, {{ $key }});">+ Add Option</a>
											</div>
										@endif
					                </div>
					            </div>
					            <div class="col-md-4">
					                <div class="form-group">
					                    <select class="form-control" id="type{{ $key }}" name="u_type[{{ $key }}]" required>
										    <option value=""></option>
										    <option @if ($value->type == 'MULTIPLE_CHOICE_RADIO') selected @endif value="MULTIPLE_CHOICE_RADIO">MULTIPLE CHOICE RADIO</option>
										    <option @if ($value->type == 'MULTIPLE_CHOICE_CHECK') selected @endif value="MULTIPLE_CHOICE_CHECK">MULTIPLE CHOICE CHECK</option>
										    <option @if ($value->type == 'ESSAY') selected @endif value="ESSAY">ESSAY</option>
										</select>
					                </div>
					                <strong>Upload Image</strong>
					                <div class="form-group">
					                    <input type="file" name="u_image[{{ $key }}]" accept="image/png, image/jpg, image/jpeg">

					                    @if ($value->image)
					                    <br><br>
					                    <img style="width: auto; height: 150px;" src="{{ asset('files/surveys/'.$survey->id.'/'.$value->image) }}">
					                    @endif
					                </div>
					                <strong>Required</strong>
					                <div class="form-group">
					                    <input type="checkbox" name="u_is_required[{{ $key }}]" @if ($value->is_required == 1) checked @endif data-bootstrap-switch data-off-color="danger" data-on-color="success">
					                </div>
					            </div>
					        </div>
					    </div>
...
@foreach($survey->question as $keyJs => $valueJs)
<script>
	$(document).ready(function() {
	    $('#type{{ $keyJs }}').select2({
	        width: '100%',
	...
	$("#type{{ $keyJs }}").on("change", function() {
		var type = $(this).val();

		if (type == "MULTIPLE_CHOICE_CHECK" || type == "MULTIPLE_CHOICE_RADIO") {
			$("#multiple_choice{{ $keyJs }}").html('<div id="show_option{{ $keyJs }}"> <test>  </test> </div> <div class="form-group"> <a href="javascript:void(0)" class="btn btn-sm btn-warning" onClick="return addOption({{ $keyJs }});">+ Add Option</a> </div>');
			$("#essay{{ $keyJs }}").empty();
		}

		if (type == "ESSAY") {
			$("#multiple_choice{{ $keyJs }}").empty();
			$("#essay{{ $keyJs }}").html('<div class="form-group"> <input type="text" class="form-control" value="Short answer text" readonly> </div>');
		}
	});
...
<script>
	...
	function addUpdateOption(questionId, counter) {
		var newRow = $("<test>");
		var cols = "";

		cols += '<input type="hidden" name="u_a_question_id['+counter+']" value="'+questionId+'"><div class="form-group"> <div class="input-group"> <input type="text" name="u_a_answer['+counter+'][]" class="form-control" placeholder="Option" required> <div class="input-group-prepend"> <span class="input-group-text"><a href="javascript:void(0)" class="ibtnDel"><i class="fa fa-trash"></i></a></span> </div> </div> </div>';
